...
class myDataSet(Dataset):
    '''
    x,y的筛选，grids的计算
    输入:处理好csv文件位置 batches*seq_length*vehicle_num行
    正常模式：
    输出tensor：x:tensor(1,seq_length-1,vehicle_num,归一化(output_col))
    输出tensor：y:tensor(1,seq_length-1,vehicle_num,2)
    输出tensor:grids:tensor(1,seq_len=99,vehicle_num=26,grids_height,grids_width)
    长时模式：
    输出tensor：x:tensor(1,seq_length/5,vehicle_num,归一化(output_col))
    输出tensor：y:tensor(1,seq_length/5,vehicle_num,2)
    输出tensor:grids:tensor(1,seq_len/5,vehicle_num=26,grids_height,grids_width)
    '''
    ...
    def __init__(self, csv_source, need_col, output_col, grids_width, grids_height, meter_per_grid, road,
        ...
        # 开始初始化
        self.init_data = pd.read_csv(csv_source, usecols=need_col)  # [batches*seq_length*vehicle_num+batches, len(need_col)]
    ...
    def __getitem__(self, item):
        ...
        提取数据，[num_batch,df(seq_length*vehicle_num,len(need_col)]
                输出tensor：x:tensor(seq_length=99,vehicle_num,9)
                9个特征[local_x,local_y,v_length,v_width,motor,auto,truck,turn_left,turn_right]
                输出tensor：y:tensor(seq_length=99,vehicle_num,2)
                输出np.array:grids:tensor(seq_len=99,vehicle_num=26,grids_height,grids_width)
        '''
        grids = []
        seq_data = []
        ...
        if not self.long_term:
            ...
            y_seq_data = seq_data[1:, :, self.col_seq['Local_X']:self.col_seq['Local_Y'] + 1]  # [seq_length-1,df(vehicle_num=26,vec=2)]
        ...
        # 长时预测
            ...
            y_seq_data = seq_data[seq_data.shape[0]//2:, :, self.col_seq['Local_X']:self.col_seq['Local_Y'] + 1]  # [seq_length/2,df(vehicle_num=26,vec=2)]
    ...
    def normalization(self, seq_data, col_seq):  # [100,26,6]
        ...
        # local_Y归一化
        self.min_Local_Y, self.max_Local_Y = np.min(seq_data[:, :, col_seq['Local_Y']]),np.max(seq_data[:, :, col_seq['Local_Y']])+20
