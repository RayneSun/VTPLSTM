...
import random
from parameters import dataExecute_conf
...
def main():
    ...
    for dist_index in range(conf.hist_dist,total_dist):
        ...
        for time_index in range(conf.hist_time,total_time):
            if conf.noise:
                time_noise=random.randint(0,200)
                dist_noise=random.randint(0,200)
            else:
            ...
            # TODO 过滤出有变道信息
            # elif one_sequence.groupby("Lane_ID")
                pass
            else:
                total_data+=1
