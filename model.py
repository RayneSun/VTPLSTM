...
class VPTLSTM(nn.Module):
    ...
    def __init__(self, rnn_size, embedding_size, input_size, output_size, grids_width, grids_height, dropout_par,device):
        ...
        self.social_tensor_conv1 = nn.Conv2d(in_channels=self.rnn_size, out_channels=self.rnn_size // 2, kernel_size=3,
                                             stride=1)
        self.social_tensor_conv2 = nn.Conv2d(in_channels=self.rnn_size // 2, out_channels=self.rnn_size // 4,
                                             kernel_size=3, stride=1)
        self.social_tensor_embed = nn.Linear((self.grids_height - 4) * (self.grids_width - 4) * self.rnn_size // 4,
                                             self.embedding_size)
    ...
    def forward(self, x_seq, grids, hidden_states, cell_states,long_term=False):
        ...
        模型前向传播
        params:
        input_data: 输入的一组数据tensor(seq_len=99,vehicle_num=26,input_size=9)
        ...
        if not long_term:
            ...
            for future_frame in range(50):
                last_point,grid=self.dataMakeUp(stable_value=self.x_seq[0,:,2:7],last_point=last_point)
                output = self.frameForward(last_point, grid=grid)
                outputs.append(output)
    ...
    def frameForward(self, frame, grid):
        ...
        social_tensor = self.getSocialTensor(grid, self.hidden_states)
    ...
    def getSocialTensor(self, one_frame_grids, hidden_states):
        '''

        :param one_frame_grids: 一帧的相关性判断矩阵tensor(vehicle_num=26,grids_height=39,grids_width=5)，没车默认-1
        :param hidden_states: 隐藏状态，tensor(vehicle_num=26,rnn_size=128)
        ...
        for one_grid in range(total_grids):
            ...
            social_tensor[target_vehicle_index][grids_height_index][grids_width_index] = hidden_states[
    ...
    def dataMakeUp(self,stable_value,last_point):
        ...
        turn_left=torch.as_tensor(combine_data[:,0]*self.road_info["max_Local_X"]>self.road_info["lane_one_max"],dtype=torch.int,device=self.device)
        turn_right=torch.as_tensor(combine_data[:,0]*self.road_info["max_Local_Y"]<self.road_info["lane_five_min"],dtype=torch.int,device=self.device)
        combine_data=torch.cat([combine_data,torch.unsqueeze(turn_left,dim=-1),torch.unsqueeze(turn_right,dim=-1)],dim=1)
